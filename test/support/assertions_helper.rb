...
module TestSupport::AssertionsHelper
  ...
  # Assert that the UPDATE query generated by the block has expected counts of things
  ...
  #
  def assert_query_sql(values: nil, on_width: nil, cases: nil, whens: nil, binds: nil, coalesce: nil, &block)
    sql_log = capture_sql_log(&block)
    updates_count = sql_log.scan("UPDATE ").size

    flunk "Expected exactly one UPDATE query, but found #{updates_count}:\n#{sql_log}" unless updates_count == 1
    ...
    unless on_width.nil?
      ...
      flunk "Expected exactly one ON clause, but found #{on_count}:\n#{sql_log}" unless on_count == (on_width > 0 ? 1 : 0)
    ...
    unless coalesce.nil?
      actual_coalesces = sql_log.scan(/[ (]COALESCE\(/).size
      assert_equal coalesce, actual_coalesces, "Unexpected number of COALESCE expressions:\n#{sql_log}"
    end
  end
  ...
  def assert_model_snapshot_delta(model, original, actual, differences)
    ...
    # Use flunk for instances of logic error in the written test itself, to avoid inflating the assertions count.
    differences.each do |id, spec|
      case spec
      when :deleted
        flunk "Expected row #{id.inspect} to be deleted, but it never existed" unless original[id]
        assert_nil actual[id], "Expected row #{id.inspect} to be deleted, but it still exists"
        expected.delete(id)
      when :created
        flunk "Expected row #{id.inspect} to be created, but it already existed" if original[id]
        assert_not_nil actual[id], "Expected row #{id.inspect} to be created, but it no longer exists"
        expected[id] = actual.fetch(id)
      when Hash
        flunk "Expected row #{id.inspect} to be updated, but it never existed" unless original[id]
        flunk "Expected row #{id.inspect} to be updated, but it no longer exists" unless actual[id]
    ...
    flunk "Found unexpected created rows" unless (actual.keys - expected.keys).empty?
    flunk "Found unexpected deleted rows" unless (expected.keys - actual.keys).empty?
