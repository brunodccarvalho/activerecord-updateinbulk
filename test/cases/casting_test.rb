...
class CastingTest < TestCase
  ...
  def test_typecast_assigns_and_conditions_string_family
    ...
    assert_query_sql(values: 2) do
      TypeVariety.update_in_bulk([
        ...
        [{ col_varchar: "v_z_3" }, { col_text: "matched by varchar z" }]
      ])
    end
    assert_query_sql(values: 2) do
      TypeVariety.update_in_bulk([
        [{ col_char: "c_d_2" }, { col_text: "matched by char" }],
        [{ col_char: "c_z_3" }, { col_text: "matched by char z" }]
      ])
    end
    ...
    assert_model_delta(TypeVariety, {
      ...
      3 => { col_string: "zeta", col_varchar: "v_z_3", col_char: "c_z_3", col_text: "matched by char z" }
    })
  end
  ...
  def test_typecast_assigns_inlined_inet_date
    skip "Adapter does not support PostgreSQL advanced types" unless postgres?

    assert_query_sql(values: 1, cases: 0) do
      TypeVariety.update_in_bulk({
        1 => { col_inet: "192.168.0.10", col_datetime: "2023-01-01 02:30:00" },
        2 => { col_inet: IPAddr.new("192.168.0.10"), col_datetime: Time.new(2023, 1, 1, 2, 30, 0) }
      })
    end

    assert_model_delta(TypeVariety, {
      1 => { col_inet: IPAddr.new("192.168.0.10"), col_datetime: Time.new(2023, 1, 1, 2, 30, 0) },
      2 => { col_inet: IPAddr.new("192.168.0.10"), col_datetime: Time.new(2023, 1, 1, 2, 30, 0) }
    })
  end
  ...
  def test_typecast_for_jsons_is_constantized_if_equal_serialization
    ...
    # Important for jsonb these do not get constantized
    assert_query_sql(values: 2, on_width: 1, cases: 0) do
      User.update_in_bulk({
        1 => { notifications: { "a" => 1, "b" => 3 } },
        2 => { notifications: { "b" => 3, "a" => 1 } }
      })
    end

    assert_model_delta(User, {
      1 => { notifications: { "a" => 1, "b" => 3 } },
      2 => { notifications: { "a" => 1, "b" => 3 } }
    })
  end
